    ...
    const fetchData = async () => {
      ...
      try {
        const { data, error: fetchError } = await supabase
          .from('projects')
          .select('dv5_titre, dv5_vizualisation_type, dv5_value, dv5_nom_icon, image_dv5, dv5_short_description')
          .eq('id', projectId)
          .single();

        if (fetchError) throw fetchError;

        if (data && data.dv5_titre) {
          setDataPoint({
            title: data.dv5_titre,
            description: data.dv5_short_description || '',
            type: data.dv5_vizualisation_type as any,
            value: data.dv5_value,
            imageUrl: data.image_dv5,
            pictogram: data.dv5_nom_icon ? {
              icon: data.dv5_nom_icon,
              color: selectedColor.value
            } : undefined
          });
          
          // Set form values
          setTitle(data.dv5_titre);
          setDescription(data.dv5_short_description || '');
          setVizType(data.dv5_vizualisation_type as any);
          if (data.dv5_value) setValue(data.dv5_value);
          if (data.dv5_nom_icon) {
            setSelectedPictogram(pictogramOptions.find(p => p.label === data.dv5_nom_icon));
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData: any = {
        dv5_titre: title,
        dv5_vizualisation_type: vizType,
        dv5_value: vizType === 'pictogram' ? null : value,
        dv5_nom_icon: vizType === 'pictogram' ? selectedPictogram?.label : null,
        image_dv5: null,
        dv5_short_description: description
      };

      // Handle image upload
      if (vizType === 'image' && dataPoint?.imageUrl) {
        updateData.image_dv5 = dataPoint.imageUrl;
      }
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="ml-64 flex-1">
        <div className="bg-white border-b border-gray-200 p-6">
          <h1 className="text-3xl font-light text-gray-900">Data Viz 5</h1>
        </div>
