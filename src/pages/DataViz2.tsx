    ...
    const fetchData = async () => {
      ...
      try {
        const { data, error: fetchError } = await supabase
          .from('projects')
          .select('dv2_titre, dv2_vizualisation_type, dv2_value, dv2_nom_icon, image_dv2, dv2_short_description')
          .eq('id', projectId)
          .single();

        if (fetchError) throw fetchError;

        if (data && data.dv2_titre) {
          setDataPoint({
            title: data.dv2_titre,
            description: data.dv2_short_description || '',
            type: data.dv2_vizualisation_type as any,
            value: data.dv2_value,
            imageUrl: data.image_dv2,
            pictogram: data.dv2_nom_icon ? {
              icon: data.dv2_nom_icon,
              color: selectedColor.value
            } : undefined
          });
          
          // Set form values
          setTitle(data.dv2_titre);
          setDescription(data.dv2_short_description || '');
          setVizType(data.dv2_vizualisation_type as any);
          if (data.dv2_value) setValue(data.dv2_value);
          if (data.dv2_nom_icon) {
            setSelectedPictogram(pictogramOptions.find(p => p.label === data.dv2_nom_icon));
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData: any = {
        dv2_titre: title,
        dv2_vizualisation_type: vizType,
        dv2_value: vizType === 'pictogram' ? null : value,
        dv2_nom_icon: vizType === 'pictogram' ? selectedPictogram?.label : null,
        image_dv2: null,
        dv2_short_description: description
      };

      // Handle image upload
      if (vizType === 'image' && dataPoint?.imageUrl) {
        updateData.image_dv2 = dataPoint.imageUrl;
      }
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="ml-64 flex-1">
        <div className="bg-white border-b border-gray-200 p-6">
          <h1 className="text-3xl font-light text-gray-900">Data Viz 2</h1>
        </div>
