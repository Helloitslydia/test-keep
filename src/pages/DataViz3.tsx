    ...
    const fetchData = async () => {
      ...
      try {
        const { data, error: fetchError } = await supabase
          .from('projects')
          .select('dv3_titre, dv3_vizualisation_type, dv3_value, dv3_nom_icon, image_dv3, dv3_short_description')
          .eq('id', projectId)
          .single();

        if (fetchError) throw fetchError;

        if (data && data.dv3_titre) {
          setDataPoint({
            title: data.dv3_titre,
            description: data.dv3_short_description || '',
            type: data.dv3_vizualisation_type as any,
            value: data.dv3_value,
            imageUrl: data.image_dv3,
            pictogram: data.dv3_nom_icon ? {
              icon: data.dv3_nom_icon,
              color: selectedColor.value
            } : undefined
          });
          
          // Set form values
          setTitle(data.dv3_titre);
          setDescription(data.dv3_short_description || '');
          setVizType(data.dv3_vizualisation_type as any);
          if (data.dv3_value) setValue(data.dv3_value);
          if (data.dv3_nom_icon) {
            setSelectedPictogram(pictogramOptions.find(p => p.label === data.dv3_nom_icon));
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData: any = {
        dv3_titre: title,
        dv3_vizualisation_type: vizType,
        dv3_value: vizType === 'pictogram' ? null : value,
        dv3_nom_icon: vizType === 'pictogram' ? selectedPictogram?.label : null,
        image_dv3: null,
        dv3_short_description: description
      };

      // Handle image upload
      if (vizType === 'image' && dataPoint?.imageUrl) {
        updateData.image_dv3 = dataPoint.imageUrl;
      }
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="ml-64 flex-1">
        <div className="bg-white border-b border-gray-200 p-6">
          <h1 className="text-3xl font-light text-gray-900">Data Viz 3</h1>
        </div>
